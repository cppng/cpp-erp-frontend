...
import { isEmailUtil, isEntryLengthUtil } from '../../utils/validationUtils';

function SignUpForm({entry, successAction, updateFormType}) {
    ...
    const postForm = ()=>{
        ...
        if(!isEntryLengthUtil(name, 2)){
            setFrmMsg("Please enter a valid name");
            return;
        }
        ...
        if(!isEntryLengthUtil(phone, 6)){
            setFrmMsg("Please enter a valid phone number");
            return;
        }
    ...
    return (
        <div style = {styles.form}>
            ...
            <div className='form-group'>
                <Link onClick = {()=>{updateFormType("LOGIN", "login")}} style={styles.loginLink}>Already have an account? Login</Link>
            </div>
        </div>
    )
